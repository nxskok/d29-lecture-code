...
with the same result. (We have to do repeated measures this way, but you can do MANOVA either way.)
...
```{r}
dogs.1=Manova(dogs.lm, idata=times.df, idesign=~times)
dogs.1
```
...
times=colnames(response)
times.df=data.frame(times)
dogs.2=Manova(dogs.lm,idata=times.df, idesign=~times)
```
...
times=colnames(response)
times.df=data.frame(times)
exercise.2=Manova(exercise.1, idata=times.df, idesign=~times)
exercise.2
```
...
times=colnames(response)
times.df=data.frame(times)
runners.2=Manova(runners.1, idata=times.df, idesign=~times)
runners.2
```
...
times=colnames(response)
times.df=data.frame(times)
runners.4=Manova(runners.3, idata=times.df, idesign=~times)
runners.4
```
